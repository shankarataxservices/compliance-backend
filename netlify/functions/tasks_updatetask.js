...
exports.handler = withCors(async (event) => {
  ...
  const newType = String(body.type ?? base.type ?? 'FILING').trim(); // free text
  ...
  const sendClientCompletionMail =
    body.sendClientCompletionMail === undefined ? base.sendClientCompletionMail : (body.sendClientCompletionMail !== false);
  ...
  for (const t of targets) {
    ...
    // Backward compatibility: if an old due event exists, patch it too (optional)
